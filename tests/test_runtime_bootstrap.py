...
from speechrail.config.model_catalog import RuntimeLock, load_runtime_lock
...
def _uv_style_role_file(requirements: tuple[str, ...]) -> bytes:
    """Render requirements the way `uv pip compile --generate-hashes` writes them."""
    lines = [
        "# This file was autogenerated by uv via the following command:",
        "#    uv pip compile --python-version 3.14.7 --generate-hashes",
    ]
    for requirement in requirements:
        first, *hashes = requirement.split()
        lines.append(first + " \\")
        for index, item in enumerate(hashes):
            lines.append(f"    {item}" + (" \\" if index < len(hashes) - 1 else ""))
        lines.extend(("    # via", "    #   fixture-parent"))
    return ("\n".join(lines) + "\n").encode("utf-8")


def test_prepare_runtime_accepts_uv_comment_headers_in_role_files(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    asr_bytes = _uv_style_role_file(_lock().asr_requirements)
    tts_bytes = _uv_style_role_file(_lock().tts_requirements)
    lock = _lock(
        file_hashes={
            "runtime/asr.txt": hashlib.sha256(asr_bytes).hexdigest(),
            "runtime/tts.txt": hashlib.sha256(tts_bytes).hexdigest(),
        }
    )
    assets = tmp_path / "assets"
    runtime = assets / "runtime"
    runtime.mkdir(parents=True)
    (runtime / "asr.txt").write_bytes(asr_bytes)
    (runtime / "tts.txt").write_bytes(tts_bytes)
    monkeypatch.setattr(bootstrap, "_ASSET_ROOT", assets)

    result = prepare_runtime(lock, tmp_path / "app", FakeRunner())

    assert (result.release / "requirements" / "asr.txt").read_bytes() == asr_bytes
    assert (result.release / "requirements" / "tts.txt").read_bytes() == tts_bytes


@pytest.mark.parametrize("role", ["asr", "tts"])
def test_committed_runtime_requirements_match_the_lock(role: str) -> None:
    lock = load_runtime_lock()

    raw, tokens = bootstrap._role_requirements(lock, role)

    assert hashlib.sha256(raw).hexdigest() == lock.file_hashes[f"runtime/{role}.txt"]
    assert tokens == tuple(getattr(lock, f"{role}_requirements"))

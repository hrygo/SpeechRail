...
def test_composition_serializes_heavy_work_when_footprints_are_unknown(
...
def test_composition_counts_both_quality_tts_workers_in_overlap_budget(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(services_module, "detect_system_memory_bytes", lambda: 8 * 1024**3)

    class DualResidentTTS:
        resident_worker_count = 2
    ...
    settings = Settings(
        ...
        tts_resident_bytes=int(1.5 * 1024**3),
    )
    services = build_app_services(
        settings,
        AppOverrides(batch_transcriber=object(), tts_synthesizer=DualResidentTTS()),
    )

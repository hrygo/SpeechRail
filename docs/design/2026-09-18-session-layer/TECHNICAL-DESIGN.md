...
| 18 | 会议页录制态**缺稿上的「静音麦克风」**（2026-09-19 走查发现）：实现侧今天没有静音路径（`MeetingSession` / `AudioSourceCoordinator` 只做启停与来源切换） | 会中想把房间里的话挡在外面时没有出口；它同时是 `SESSIONS-SPEC` §8 表里的 `M` 键 | 一次真实的麦克风会话（做与验一起） |

...
**v2.0.0（本版，active）是通读全部设计包、并按本地与外部证据复核后的终态**。与 v1.1.0 的实质差异见
§13.3。**v2.1.0 记录 `T1`–`T5` 五条技术裁决（用户 2026-09-18「全面采纳」）**，其中 `T5` 的三条 schema
加法已并入规格 §15.7 的 R2。**v2.2.0 增加 `T6` 存储分区与重装契约**（§6.6）。**本版无待裁决项。**
...
| `source_lost` | 被 tap 的 App 退出 | **唯一自动接回**：按 bundle id 接回，只记一条区间 |
...
- `AGENTS.md:33` 的「不采集/播放音频」句改写（`T3` 已裁决，本阶段执行）。
- `docs/developers/macos-app-audio-capture.md` 补会话级采集（连续流、电平、设备选择、释放时点）。
- `docs/architecture/current-boundaries.md` 记一条边界变更。
- 规格 §5.3.1 里指向本文的 §参考号与本版一致（防漂移，见 §13.3）。

## 12. 未验证清单

本轮把 v1.1.0 的 10 条与这一轮新出现的合并为 15 条。**没有一条能在文档里被当成已成立**；

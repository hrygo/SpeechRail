---
title: "SpeechRail 会话层技术方案 · 语音助手 / 会议助手 / 实时字幕"
status: active
...
**这一版是什么。** 它是语音助手、会议助手、实时字幕三条能力的**当前技术方案**：描述三者同时在位时系
...
**v2.0.0（active）是通读全部设计包、并按本地与外部证据复核后的当前方案**。与 v1.1.0 的实质差异见
...
| 音色（`custom-voices.json` + `voices/`） | **服务** | 当前仍在 `~/.speechrail`；迁入 app home 尚未实现 | 不动 |
...
**三项尚未完整实现，构成本轮之后的第一批维护项**（都不属于三条能力的功能范围，可独立排期）：
① 音色路径搬迁（**兼容逻辑必须先落地**：新路径优先、旧路径存在则自动搬一次并留日志、旧目录留空壳）；
② 完整数据区备份（现在设置里只有「备份记录库」，只覆盖 `sessions.sqlite3`）；③ 可重建区的 Time Machine 排除
...
**不在上表的独立项**（用户 2026-09-18 裁决，见 §6.6）：音色路径搬迁（含旧路径兼容）、完整数据区备份、
...
**没有变**的：R1–R3 三条判据、服务不动的结论（R2 + 红线）、三条能力的定义、SQLite 的落点与
迁移策略、四类中断的处置。
...
| **T3** | `AGENTS.md:33` 改写为「只在会话模块与音色克隆页采集；播放只用于会话与试听」 | **已完成**（阶段 9） | App 会话采集、XPC 采集服务与音色试听路径已落地 |
...
| **T6** | 存储分区与重装契约（§6.6） | **分批**：目录契约已写进 `docs/operations/runtime-deployment.md`；会话库单文件备份已存在；音色搬迁、完整数据区备份、Time Machine 排除三项待实现 | 桌面 App 的用户资产只有"重装不丢"才成立；单文件备份不覆盖作品、音色与配置 |

**T3 的历史说明已结清**：阶段 9 已落地 App 会话采集、XPC 采集服务和对应的仓库边界说明；后续只需按
真机声学验收项补充实测证据，不再把会话能力描述为尚未实现。

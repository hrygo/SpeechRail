...
本轮把 v1.1.0 的 10 条与这一轮新出现的合并为 16 条。**没有一条能在文档里被当成已成立**；
...
| 16 | 全局热键的**安装时机**：接线挂在菜单栏标题这个宿主上（`App.swift` 的 `MenuBarExtra` label），理由是窗口可以被关掉；但"启动后从未打开过窗口时 `.task` 是否已跑、四个键是否已生效"没有真机验证过 | 关着窗口时 `⌘⇧L` / `⌘⇧N` 是否可用 | 阶段 8 真机走查（关窗后按一次） |

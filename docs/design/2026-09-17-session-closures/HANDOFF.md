...
**最近一次改动：第七轮（2026-09-18）**——产品经理 / 全新用户双视角审查后的十处优化，全在生成器里
（`figma-kit/main.js`），离线门禁全过并重出了两份产物包；**Figma 文档尚未重建**（重跑插件需当次
授权），所以下面这份交接包的画板内容描述的是 07:36 那一版，代码落点见「第七轮 delta」一节。

生成时间：2026-09-18（第五轮）。用户逐块审查后提了 **9 条**，逐条落地：新增 2 块画板
（`换音色 vs 换人设（影响面）`、`浮层 · 字幕带 · 贴在画面上（在用时）`）、重画 3 块（助手对话页的
输入区、换音色板、会议页三块状态）、把会议页的骨架提升成两稿共用实现（删掉 2 份重复定义）、
并把「面向终端用户」定成写文案的规矩。稿从 49 板 → **53 板**。
**这一版已在 Figma 实跑并收敛**：2026-09-18 07:36 的最后一次运行是
`AUDIT VERDICT · 53 frames · all clean`（0 根溢出 / 0 内部溢出 / 0 未绑定占位灰 / 248/248 连线），
收敛过程与四次运行见下面「Figma 实跑报告（53 板这一版）」。**按用户指示仍未导出。**
...
| 稿的产物 | 新 Figma 文件（URL `figma.com/design/sAPdzrT2zsxqLtJhUx4iVC/…`，桌面版标签名被用户改为 **`Sona2Speech`**），页 `01 闭环`。**文件里现在是 53 块**，与生成器同步（2026-09-18 07:36 运行时重建，该次报告为 `all clean`） | 插件包在 `~/Downloads/SpeechRail-closure-kit/`（`code.js` md5 `7b1bca6e074d26c2ec4ab7b9a7759da7`，2026-09-18 第五轮收敛版） |
...
## 第七轮 delta：产品经理 / 全新用户双视角审查（2026-09-18，改稿未实跑）

用户：「再次从一个产品经理视觉，一个全新用户视觉，审查设计稿」→「执行优化」。十处发现全部改在
生成器里；规范位置 [`SESSIONS-SPEC §17`](../2026-09-17-live-sessions/SESSIONS-SPEC.md)，逐条复核
[`USER-JOURNEYS.md §13`](USER-JOURNEYS.md)。**Figma 里的稿仍是 07:36 实跑收敛的那一版**——这一轮
只改了生成器与产物包，重跑插件（需当次授权）之后稿才会跟上。
...
**未跑几何**：两个 smoke 都不检查几何（替身没有布局引擎）。本轮对结论条、检查器、状态带的增删
都按"净减少行数"设计（去重 7 处、瘦身 2 处、新增 1 行检查器 kv 由删掉的 3 行说明抵掉），但折行
与高度差只能由一次实跑证明。
...
Figma 实跑已在 2026-09-18 07:36 做完并收敛（用户当时的「授权」覆盖了运行插件，未覆盖导出）；
**导出仍留到用户下一次开口**。
...
**注意**：Figma 文件里现在是 **53 块**（第五轮那一版，`7b1bca6e074d26c2ec4ab7b9a7759da7`）。
第七轮改过 `main.js` 之后，插件包已经重出（md5 `c72bf2e9b8077a8ba98a21366e649861`），
**两者暂时不一致**：稿要等一次热重载 + 运行（需当次授权）才会跟上，流程不变。
...
- 上面三道离线门禁 + **两次** Figma 实跑的 `AUDIT VERDICT · all clean`：43 板那一版 2026-09-17 22:02，
  53 板这一版 2026-09-18 07:36（后者经历 4 次运行收敛，过程记在「Figma 实跑报告」一节）。
...
- **Figma 实跑已完成**（2026-09-18 07:36，`AUDIT VERDICT · 53 frames · all clean`）。
...
- **这一版（53 板）没有在 Figma 里实跑过**：板数与连线数只由离线替身确认；几何（root / inner
  overflow）在实跑前没有证据。第四轮的 2 块新板刻意对齐了已收敛的骨架（页头 + 状态带 + 两栏，

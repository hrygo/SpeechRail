...
| # | 优化项 | 旅程触点 | 代码落点 |
|---|---|---|---|
| 1 | **J1 打字输入静音预期强化**：输入框 placeholder 明示静音不外放 | J1 语音助手文字交互（消除公共外放焦虑） | `main.js:3334`：`placeholder: "输入消息（↵ 发送 · 静音不外放）…"` |
| 2 | **J2 会议来源自动混音心智理顺**：消除第 4 行伪未选状态，重构为自动混音状态说明行 | J2 会议来源配置（消除认知歧义） | `main.js:4494`：重构为说明行 `["Ready", "自动混音", "多来源同时勾选时自动合流处理…", "生效中"]` |
| 3 | **J2 会议整理期紧急导出通道**：会议整理中新增 `先导出转录…` 按钮直通归档导出 | J2 会议结束整理期（消除等待大模型阻塞） | `main.js:4232`：增加 `secondaryButton(row, "先导出转录…", "download")`，`main.js:5999` 注册原型跳转连线 |
| 4 | **连线门禁扩展核查**：声明连线源从 52 增加至 53，100% 解析 | 原型连线可靠性守卫 | `check-links.js closures`：53/53 连线源全通过 |

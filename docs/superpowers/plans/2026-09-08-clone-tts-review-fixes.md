...
**Architecture:** Keep the existing request-scoped PCM16 controller and 200 ms bounded clone buffer. When the 240 ms calibration window completes mid-stream, use its bounded gain as the baseline for that transition once, then return to live chunk tracking. Keep the 200 ms framing private and verify that it preserves sample order/count while explicitly measuring its latency-sensitive boundary.
...
- Produces: A regression test proving that calibration completed after the second chunk becomes the active request baseline.

- [ ] **Step 1: Write the failing test**

  Feed a 200 ms quiet chunk followed by a 200 ms loud chunk and assert that the current request gain equals the computed calibration gain after the calibration window completes.

- [ ] **Step 2: Run the focused test and verify it fails**

  Run `uv run --extra dev pytest --no-cov tests/test_tts_loudness.py -q`.

  Expected: the new test fails because `_current_gain_db` remains the live chunk target instead of the completed calibration target.

### Task 2: Apply calibration once at the transition
...
- Produces: a bounded one-time calibration baseline with existing peak ceiling and reset semantics.

- [ ] **Step 1: Add request state for one-time calibration application**

  Reset a `_calibration_applied` flag with the rest of request state.

- [ ] **Step 2: Use the bounded calibration gain when it first becomes available**

  After `_advance_calibration()`, if active audio exists and calibration is complete but has not been applied, select `_calibration_gain_db` as the current desired gain for that chunk and mark it applied. Subsequent chunks continue using live RMS control.
...
- [ ] **Step 1: Extend the fake clone test with output-size assertions**
...
- [ ] **Step 3: Keep the framing constant private**
...
- [ ] **Step 1: Add aggregate-only final acceptance evidence**
...
- [ ] **Step 2: Mark the Sona record as superseded by the final verification**

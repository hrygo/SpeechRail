...
| W11 逐档声学/性能与发布回滚 | in_progress（四档真实增量门已过） | 2026-09-25：cp314 wheel 装成 managed runtime（`2b0eb8ff…`，回退点 `99bb218a…` 与 cp312 `b6d3394c…`），修复 client `_publish` 丢事件与 worker `_emit` 丢 `on_sent` 两处缺陷后，light/balanced/quality/extreme 四档真实 WebSocket + 真实模型基准 `failures: []`（短句 first PCM p50 23.2–55.5 ms、RTF p50 0.199–0.300；254 字长句与 400 ms 四段追加同样通过）；Base 参考音频需 ~2.4 s 级；cancel 真机共暴露三层缺陷：已完成 utterance 的残留帧污染下一轮（worker `StreamPump` 把取消帧既入队又置优先级标志）、open 失败 abort 后 worker 仍标记 started（放大成必须重启服务）、以及父端 `cancel()`/`close()` 在没读走终态时按“终态已知”收尾把该帧留在共享 wire 上（轮询交替失败的真正来源），三层均已源码修复并加确定性回归；带修复的 wheel（`dc83d911…`）装机后 `--mode cancel --repeat 5` 连跑 5 轮 **25/25 全部取消成功**、`failures: []`、`stale_audio_turns: 0`、`cancel→terminal` p95 6.3–24.9 ms（预算 500 ms）、`next_start_accepted` p95 12.7–31.0 ms，期间 `fallback_abort_count=0`/`reload_count=0`（走协作取消，不再杀子进程兜底）。长稳 soak 已在 quality 档跑通 40 轮（40/40 完成、40/40 取消、空转取消探针全返回 `tts_not_active`，`failures: []`；governor release +120、活跃请求峰值 0、服务进程数恒为 2；足迹首轮预热到 3662.8 MB 后 39 轮仅在 3661.1–3663.1 MB 波动，读数取自服务自身 `footprint` 口径且 complete=1；期间还修掉 soak 工具自己把 release probe 终态留在 socket 上的缺陷）。未运行：真实播放欠载、打断→停旧音时延、light/balanced/extreme 的 soak、跨文本身份 A/B、light/balanced 的 ASR 端到端、extreme 资源峰值、App 可听与发布回滚演练 |
...
W11 真实增量门（2026-09-25）：cp314 wheel 已装成 managed runtime（`2b0eb8ff…`，保留 `99bb218a…` 与 cp312 `b6d3394c…` 回退点），并在修复两处真实缺陷后跑通四档真实基准。缺陷一在客户端：`qwen3_tts_stream_client._publish` 丢弃非终态事件，改为对 worker 施加背压；缺陷二在 worker：`_emit` 重建 `StreamFrame` 时丢掉 host 帧的 `on_sent`，`pending_audio_bytes` 只增不减，第 13 个 3840 B chunk 必越过 48000 B 预算并报 `tts_backpressure`（原始 wire trace 与 `48000/3840=12.5` 吻合，现已用确定性测试钉住）。四档短句结果：light 23.3 ms / RTF 0.208、balanced 23.2 ms / 0.208、quality 43.4 ms / 0.237、extreme 55.5 ms / 0.300，`failures: []`。身份与精度差异仍是分档核心：light/balanced 为 CustomVoice 内置 speaker 且 `tts_clone=None`，quality/extreme 为 Base clone，prepared reference 按量化隔离，同一 voice id 在两个精度下各自准备参考。Base 参考音频需 ~2.4 s 级，8.4 s 参考会触发 `prefill_did_not_enter_trailing_region`。cancel 真机运行共暴露三层缺陷，逐层修复后才闭环：① worker `StreamPump` 把取消帧既入队又置优先级标志，模型循环走优先级分支时未消费该帧，残留帧被下一次 `_serve_frames` 按无活动 utterance 答复，生成携带旧 `request_id` 的错误帧使新 utterance fail-closed；② open 失败 abort 子进程后 worker 未像 batch 路径那样失效 `_started`，导致后续请求持续写死管道——这两处修掉后真机仍交替失败；③ 父端 `cancel()` 的 `_await_dispatcher` 吞掉超时且不返回结果、`close()` 只在 `_fatal` 时 abort，于是“没读走终态”被当成“终态已知”，那份 `tts_stream_done` 留在共享 wire 上，被下一轮当成外来帧 fail-closed。修复为 `discard_ended_stream` + 失败后 `_invalidate_after_abort`、`_await_dispatcher` 返回 `bool` 且未 drain 即 abort、`close()` 无终态即 abort，另加一层纵深防御：worker 用有界记录记住本进程开过的增量 `request_id`，客户端丢弃属于**已结束** utterance 的帧（未知外来 id 仍 fail-closed，且错误信息带 `type/request_id/expected`）。新增 4 项确定性回归；`-k "tts or realtime or stream"` 786 passed，改动文件 `ruff check` 与 `mypy --strict` 干净。真机装上 `dc83d911…` 后 `--mode cancel --repeat 5` 连跑 5 轮 = **25/25 取消成功**、`failures: []`、`stale_audio_turns: 0`，期间 `fallback_abort_count=0`/`reload_count=0`（详见 Luna 指南 W11 第二轮小节）。**尚未验收：** 真实播放欠载、打断→停旧音时延、跨档 soak 长稳、跨文本盲听身份 A/B、light/balanced 的 ASR 端到端、extreme 资源峰值、App 可听与发布/回滚演练；extreme 仍按候选档呈现，不因基准通过自动转正式实时档。

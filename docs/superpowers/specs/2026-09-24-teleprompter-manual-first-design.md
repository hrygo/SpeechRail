...
- `TeleprompterStageView` 将阅读层与控制层分开；计时/采集需要时显示 24pt 辅助条，底部控制区固定预留 48pt；隐藏只撤内容与无障碍子树。
- 舞台支持 1／2／3 条实际显示行：1 行显示当前行，2 行显示当前行和下一行，3 行显示上一已读行／当前行／下一行；当前行固定在三行模式的中间。长稿按舞台可用宽度自然折行为显示行，行位置映射回原稿的段落与 UTF-16 偏移；全稿浏览仍保留完整段落、滚动和文本选择。
- `TeleprompterStageWindowController` 的用户关闭、系统关闭与程序关闭统一走现有关闭生命周期；用户显式打开时使舞台可接收局部快捷键，内容更新与后台语音不得抢焦点。绿色缩放只横向铺满可用宽度，窗口最大高度 360pt；前置已打开窗口不重置用户尺寸。
...
| D05 | 空格执行下一显示行 | 不随语音状态改变语义，不隐式开启麦克风 |
| D06 | 末行仍是台词 | 到末行/读完末行/反复下一行均不自动展示总结或关闭 |
...
- 一级控制：**上一行、下一行、语音跟随、显示设置、关闭**。不把“开始/暂停提词”作为入口，因为台词从打开时就已可读。
- 左/上/PageUp：上一显示行；右/下/PageDown/空格：下一显示行；Home/End：首行/末行。边界不循环、不越界；到边界的按键不改变阅读位置，也不隐式关闭舞台。
...
- 正常阅读区按可用宽度和字号计算实际显示行，行布局只在稿件、字号或可用宽度变化时重算；切行定位到对应原稿 UTF-16 起点，不能丢字、重复或截断。阅读行区不通过手势滚动暗中推进阅读位置；「查阅全稿」继续按完整段落滚动并可选择文本。
- 三行布局始终保留上一已读行／当前行／下一行的固定槽位；稿首缺少上行或稿尾缺少下行时保留空槽，当前行不得跳位。快捷键、控制栏按钮和应用菜单都使用同一行级定位路径；菜单快捷键为 `⌘⌥←/→`，不新增全局热键。
- 从首行执行上一行或从末行执行下一行不改变位置、不循环。实际行定位属于手动接管；若语音正在工作，则同步撤销旧推进权并停止语音。正文选择、鼠标移入移出、字号改变不算手动定位。
...
- 既有字号和背景透光快捷键可保留；菜单栏提供与舞台同源的上一行/下一行/语音跟随/显示设置/关闭命令，不重复实现 Session 状态逻辑。
...
- **阅读层**：按可用宽度排成 1／2／3 条实际显示行；三行时上行表示已读内容，中行是当前阅读行，下行预览下一行。行级布局保留原稿 UTF-16 映射；正文不随控件一起变透明。
...
- 文字对比度优先，背景透光与文字透明度分离；下一显示行只降视觉层级，不得降到不可读。
...
- 鼠标停在正文内不保持控制区显示；指针进入操作区立即显示，离开后按延迟隐藏。本期不增加猜测性 idle 超时。
...
保留字号、背景透明度与既有预设。背景透明度范围为 0–100%；达到 100% 时阅读区底色、材质、描边和当前行底板完全隐藏，但正文、焦点提示和底部控件不跟随变透明。设置以 1／2／3 条作为“显示行数”选择，默认沿用当前有效默认值 3；1 行显示当前行，2 行显示当前行与下一行，3 行显示上一已读行、当前行与下一行。三行模式在稿首／稿尾保留空槽并保持当前行居中。自动折行依据当前字号和正文可用宽度计算，并保存逐行到原稿的段落与 UTF-16 映射；单次翻行不重排全文。全稿浏览仍显示完整段落并可滚动选择。设置改变即时更新窗口首选高度，不重置阅读位置或停止语音。
...
| `.../TeleprompterStageWindow.swift` | 窗口生命周期、用户显式打开时的键盘焦点、横向标准缩放、高度上限和尺寸恢复 |
| `.../TeleprompterStageSettings.swift` | 计时/控制偏好、1–3 条显示设置与纯几何策略，不改稿件 schema |
...
| `.../SpeechRailDesignTokens.swift` | 集中维护紧凑边距、条件辅助区 24pt、控制区 48pt、背景透明度范围和 360pt 舞台高度上限 |
...
| A04 | following；手动定位到任意显示行；注入旧 partial/final | 新段落索引与 UTF-16 偏移不变，旧 generation 不能推进或覆盖错误状态 |
...
兼容影响须明确说明：空格从开始/暂停语音改为下一显示行；舞台方向键和菜单行快捷键将阅读位置移动到显示行的原稿 UTF-16 起点；R 不再隐式重读并开启语音；Command-A 恢复原生语义；默认打开不再自动跟读；舞台不再自动展示节奏总结。这些是有意的产品行为变化，不保留隐藏的旧快捷键兼容分支。
...
| 提词器显示行/手动定位全量回归（后续重跑） | `rtk swift test --disable-sandbox --package-path macos/SpeechRailApp --filter Teleprompter`：144 tests / 15 suites 全部通过；覆盖行布局、UTF-16 映射、透明度端点、窗口几何与手动接管 | Runner 输出 2026-09-25 00:25 CST（本轮重跑）；与本会话适用日期不一致，详见下方日期说明 |
| 当前 App Debug 构建 | `rtk scripts/macos_app_build.sh --configuration Debug`：`BUILD SUCCEEDED` | 本轮执行 |
| 本轮文档更新后的格式检查 | `rtk git diff --check` 退出码 0；未跟踪计划文档检查仅发现两行用于 Markdown 硬换行的尾随空格 | 本轮执行 |

**日期来源说明：**本会话适用日期为 2026-09-24；先前本机 `date` 输出 2026-09-25 00:13 CST，本轮 Swift Testing runner 输出 2026-09-25 00:25 CST。此处保留工具原始时间并明确记录冲突；不把 runner 时间改写为 2026-09-24。

...
1. “大模型（全局默认）”：保留现有 endpoint/model/global Key/Responses-only/连接检查。
2. “模块专用配置（可选）”：助手、会议纪要、AI 提词器各一张统一样式的设置卡。

每张卡包含：跟随全局/使用专用配置开关、endpoint、model、专用 Key、当前生效来源和配置状态；专用 Key 留空明确表示继承全局 Key。所有间距、颜色、字体、圆角和控件形态复用 `SpeechRailDesignTokens`，不添加局部 token。

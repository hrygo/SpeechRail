---
title: "SpeechRail macOS App 管理控制台、模型管理与运行监控"
status: proposed
audience: "SpeechRail macOS App 开发者与本机运行维护者"
version: "0.1.0"
date: 2026-09-13
---

# SpeechRail macOS App 管理控制台、模型管理与运行监控

## 1. 目标

在现有 SpeechRail macOS 控制面上补充一个面向本机维护者的管理区域，吸收
设计交付包中的浅色原生 macOS、左侧导航、内容优先和状态分层语言，同时提供：
...
本设计不把管理能力混入未来的配音台、音色库和我的作品创作主流程。当前仓库
尚未实现创作工作台，因此本轮只建立可独立交付的管理区，不伪造未实现的创作页面。
...
### 3.1 采用：现有 Settings 窗口扩展为管理控制台

使用 `NavigationSplitView` 承载管理区，在同一个 AppModel 中统一服务控制、模型操作和监控
采样。优点是复用现有 XPC、`URLSession`、菜单栏入口和 UI test fake，避免额外窗口与状态
同步问题；模型下载仍由 helper/CLI 执行，符合现有安全边界。
...
侧栏底部固定显示简化的“语音服务状态”，菜单栏保留快速启动、停止和打开管理控制台。
未来的配音台、音色库和我的作品可以复用同一视觉骨架，但不在本轮添加空壳入口。

...
> **方案定位**：改善 SpeechRail `/v1/realtime` 的服务端 VAD（Voice Activity Detection）能力，使其在质量、维护性与契约保真度上对齐 2026 年实时语音 Agent 行业最佳实践。D1–D3 已落地；barge-in 决策属于调用方，本文不再定义服务端取消或冷却。
>
...
- 交付可被调用方消费的 VAD 事实；播放、AEC、barge-in 与冷却由调用方负责；
...
| **接线层** | `src/speechrail/application/realtime_openai.py` | 接入 current-only `server_vad`：hypothesis / text final 事实、自动 commit（`vad_stop`）；barge-in 由调用方根据事实显式发送 `speechrail.tts.cancel`，服务端不自动取消 TTS |
...
SpeechRail 的 `server_vad` 字段支持通用默认值 `threshold=0.5`、`prefix_padding_ms=300`、`silence_duration_ms=400`；这是缺省协议值，不代表所有客户端业务都使用相同窗口。调用方可以在 `session.update` 的 `session.speechrail.endpointing` 显式传递该策略（官方 `audio.input.turn_detection` 目前只接受 `null`/`"manual"`），SpeechRail 负责校验、评分和边界状态机。
...
| **G4** | 旧设计把全双工打断放在服务端 | 当前契约要求服务端只发 `speech_started/stopped` 事实，调用方显式发送 `speechrail.tts.cancel` | ✅ 已按当前边界关闭 |
...
### D4 · Barge-in 边界：调用方拥有取消决策

**决策**：SpeechRail 只发送 hypothesis 与 text final 事实（当前 wire 没有独立的 speech
start/stop 事件），不保存播放状态、不发送 `speechrail.tts.cancel`，也不提供服务端 barge-in cooldown
配置。Native、Sona 或其他调用方结合
播放队列、AEC、回声策略和业务状态决定是否取消；这避免把调用方的播放事实伪装成服务端状态。
...
| 阶段 | 内容 | 出入口 |
|---|---|---|
| **P0 前置（契约+测试）** | 在 `contracts/realtime-openai.md` 增补 `auto` 档、v5/v6 支持与 `threshold` 语义；新增/更新 VAD 事实与调用方显式取消回归 | 失败回归测试先红 |
...
| **P3 D3/D4** | `contracts/realtime-openai.md` 语义文档化；确认 VAD 事实与调用方显式取消边界 | 契约门 + caller-owned barge-in 回归绿 |
...
- [x] barge-in 边界生效：重复 `speech_started` 不会由服务端取消 TTS；调用方可显式发送匹配的 `speechrail.tts.cancel`。
...
| 面 | 影响 |
|---|---|
| **配置** | `realtime_vad_engine` 新增 `auto`（默认）；managed wheel 固定 `onnxruntime==1.29.0` |
| **契约** | `contracts/realtime-openai.md` 更新 v5/v6 支持、`threshold` 语义、`auto` 与 caller-owned barge-in 边界；`/health`/`/readyz` 提供 VAD 子能力状态 |
| **代码** | `neural_vad.py`（schema 感知）、`config/__init__.py`（引擎解析）、`realtime_openai.py`（VAD 事实与 caller-owned cancel 边界）、`application/services.py` 与 `service/preflight.py`（诊断与安装门） |
| **测试** | `test_neural_vad.py`（v5/v6、auto 三态、缺 runtime 会话回归）、`test_realtime_vad_bargein.py`（显式 cancel 边界）、preflight/packaging/health 契约测试 |
...
- **外部最佳实践（2026）**：
  ...
  - Picovoice 2026 VAD 指南与多篇工程文：能量/WebRTC VAD 在高噪音下 FPR 高、不适合生产；Silero F1≈0.86；双阈值迟滞/longest-run 平滑；trailing padding 200–300ms；调用方的 barge-in 可按播放期/静音期分开校准、用能量门限与约 250ms cooldown 防 echo-barge-in；这些参数不转化为 SpeechRail 服务端状态；语义 endpointing F1≈0.92。

...
- [x] **Step 5: 将 voice-realtime 字幕与交互 ASR 指向 SpeechRail Realtime v2，并禁用 legacy `/asr`。**
...
### Task 3: QwenPaw 与 voice-realtime 端到端验收（已完成）
...
- [x] **Step 2: 验证 voice-realtime 字幕与交互配置均加载 `speechrail-realtime-v2`。**
...
### Task 4: 删除 voice-realtime 源码中的旧 ASR adapter（独立后续任务，未开始）
...
`voice-realtime` 仓库内尚未删除的兼容代码。删除会破坏其现有公开配置和基准工具，应作为

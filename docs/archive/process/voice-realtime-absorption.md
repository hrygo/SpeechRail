...
> 与[迁移 Runbook](../../operations/migration-runbook.md)取代；其中“Sortformer 留在 `sona`”的结论不再有效。
> Qwen3 native worker 已独立实现；WLK streaming adapter、legacy snapshot parity 和
> `sona` 侧的改动均尚未实施。当前 `/asr` 仅支持 config/EOF 骨架，不能据此切换
> 旧服务。实际执行顺序见 [迁移 Runbook](../../operations/migration-runbook.md)。

## 1. 核心结论

SpeechRail 吸收的是“ASR 基础设施能力”，不是把 `sona` 整个仓库复制成
第二个综合应用。两者关系最终应是：

```text
sona = 音频采集 / 会议 / UI / TTS / 应用编排
SpeechRail      = ASR runtime / queue / public API / compatibility
```

现有 `sona` 在 2026-08-31 核对为 `1.4.0`，当前开发分支为
`feature/physical-output-audio`。它的字幕服务默认使用 WLK `8001`，并已经拥有
ASR 领域契约和 Qwen3 隔离 worker；这些是迁移输入，不能继续以 `sona.*`
作为 SpeechRail 的内部依赖。
...
|---|---|---|---|
| `src/sona/asr/contracts.py` | 端口 | `src/speechrail/domain/contracts.py` | 去掉对会议 `TranscriptWindow` 的依赖 |
| `src/sona/asr/profiles.py` | 端口并重命名 | `src/speechrail/config/profiles.py` | 只保留 ASR runtime 字段 |
| `src/sona/asr/registry.py` | 端口 | `src/speechrail/runtime/registry.py` | 独立能力门禁 |
| `src/sona/asr/adapters/qwen3_native.py` | 端口并去应用耦合 | `src/speechrail/backends/qwen3_native.py` | batch 推理和 worker lifecycle |
| `src/sona/asr/workers/qwen3_native_worker.py` | 端口并改 worker module | `src/speechrail/backends/qwen3_worker.py` | 独立 Python/依赖/模型隔离 |
| `src/sona/asr/adapters/wlk.py` | 端口 | `src/speechrail/backends/wlk_streaming.py` | 将 WLK full snapshot 规范化 |
| `src/sona/subtitles/events.py` | 仅提取 wire parser | `src/speechrail/compatibility/wlk.py` | legacy serializer/parser 边界 |
| `src/sona/asr/presenters.py` | 端口兼容函数 | `src/speechrail/compatibility/presenters.py` | 重建旧字幕 payload |
...
| `sona.meeting.*` | 不吸收 | 留在 `sona` | 会议事实源与业务生命周期 |
| `sona.ui.*` | 不吸收 | 留在 `sona` | UI、AudioHub 和模式协调 |
| `sona.tts_bridge.*` | 不吸收 | 留在 `sona` | TTS 与 ASR 是不同产品边界 |
| `sona.interaction.*` | 不吸收 | 留在 `sona` | Agent/LLM/prompt 不是 ASR 服务职责 |
| PostgreSQL / Sortformer | 不吸收 | 留在 `sona` | 数据持久化和说话人分离由会议应用拥有 |

## 3. 必须解除的耦合

### 3.1 领域对象

当前 ASR 契约导入 `sona.meeting.models.TranscriptWindow`。迁移时改为
...
当前 Qwen3 worker 命令通过 `sona.asr.workers.qwen3_native_worker` 启动。
...
SpeechRail 完成 parity 后，`sona` 只需做三类小改动：
...
不能通过让 SpeechRail import `sona` 来“复用”实现；那会把综合应用的
依赖、版本和发布周期重新引入公共服务。
...
| Sortformer | `sona` | 可选 speaker 字段，不做会议映射 |
| SRT / PostgreSQL | `sona` | `verbose_json` 或 realtime segments |
| 回声抑制 | interaction pipeline | 纯 ASR 服务不吞掉音频帧 |
| TTS 与 LLM | sona / LM Studio | SpeechRail 不接管 |
...
- `sona` 的字幕、会议 confirmed 文本、SRT 和数据库写入行为不回退。

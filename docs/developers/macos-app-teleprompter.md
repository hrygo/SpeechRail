...
- **节奏与状态看板 HUD**：舞台顶栏集成动态拾音/跟读状态胶囊、细窄进度条、语速节奏指示胶囊（`stagePaceIndicatorPadding*`）与轻量运行计时器（显示已朗读时间与预估剩余时间/超时警示），供演说者精准把控时长；
- **全生命周期有温度的演说旅程（5 阶段深度赋能）**：
  1. **登台前候场就绪**：顶栏呈现「深呼吸 · 空格从容开讲」，支持全稿浏览选定起讲锚点，一键敲击空格无缝开讲，为演讲者提供沉稳的心理托底；
  2. **冷启动开讲**：未拾取到首字前呈现温和呼吸动效的「随时开讲，我在静静聆听…」，捕捉到第一句瞬间变为翡翠绿呼吸光晕「跟读咬合 · 渐入佳境」，消除开讲紧张与失控感；
  3. **跟读行进与脱稿应对**：
     - **语速节奏教练**：基于进度与时间比率动态反馈有温度的陪伴式指引（「从容稳健 / 步调微快 / 温和舒缓 / 感知节奏中」），余光一扫即可获悉节拍与暖心建议；
     - **演说呼吸与目光交流引导**：停顿标签采用有温度的演说指导理念——短停顿标记「微歇换气」（风羽图标）、中停顿标记「留白蓄力」（星芒图标）、长停顿贴心提醒「眼神互动」（目光图标，提醒演讲者抬头微笑互动，把握舞台气场）；
     - **脱稿随心发挥托底**：演讲者脱稿互动或解答提问时，温和呈现「✨ 随心发挥中」与「尽情自由发挥，讲稿在为你静静守候 · 读回屏幕文字即可自然归队」，尊重演说者的临场即兴发挥；
  4. **暂停与互动查稿**：暂停或手动模式下提供「查阅全稿 / 聚焦两段」切换（⌘A），段落悬停呈现柔和高亮与「由此段起讲 ↵」直观锚点，支持演说者在全稿中滚轮查阅前言后文并选段起讲；恢复跟读时平滑收束聚焦至目标段落；
  5. **完稿仪式感与复盘沉淀**：演说结束呈现「✨ 精彩演说！你顺利完成了整篇讲稿」仪式感复盘卡，包含专注用时（分秒不差/从容留白/饱满展开）、实际语速（WPM）、讲稿达成段数及台风节拍，并附有温度的「💡 语速贴心小记」，支持一键采纳实测语速沉淀为个人节拍系数（`calibrationFactor`），底部提供「重新演练」与「大功告成」按钮，闭环提升演说信心。
- **盲操与舞台快捷微调**：
  ...
  - **舞台内透明度快捷切换**：操控栏提供 4 档即时透明度菜单（通透 60%、平衡 75%、清晰 90%、纯色 100%），演讲过程中无需切回主应用即可随心调节遮挡度；
  - **演说翻页笔与键盘全域盲操**：除点击外，深度支持主流蓝牙/无线演示翻页笔（`↑` / `↓` 上下段翻页），提供 `⌘=` / `⌘-` 字号缩放、`⌘A` 全稿查阅切换，以及复盘小结页敲击空格键（`␣`）直接重新开讲；

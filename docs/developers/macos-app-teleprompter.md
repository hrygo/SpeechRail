...
- **全生命周期演说旅程（5 阶段深度赋能）**：
  1. **登台前候场就绪**：顶栏呈现「候场就绪 · 空格开讲」，支持全稿浏览选定起讲锚点，一键敲击空格无缝开讲；
  2. **冷启动开讲**：未拾取到首字前呈现温和琥珀色「等待声音请开讲…」，捕捉到第一句瞬间变为翡翠绿「跟读咬合」，消除开讲疑虑；
  3. **跟读行进与脱稿应对**：
     - **语速节奏指示**：基于进度与时间比率动态反馈「节奏平稳 / 偏快 / 偏缓 / 测速中」，余光一扫即可获悉语速状态；
     - **脱稿心理托底**：未匹配到稿件内容时展示温和的「自由发挥中 · 读回屏幕文字将自动跟随」，替代刺眼的警告红黄标，让演说者放心展开现场互动；
  4. **暂停与互动查稿**：暂停或手动模式下提供「查阅全稿 / 聚焦两段」切换，支持演说者在全稿中滚轮查阅前言后文并选段起讲；恢复跟读时平滑收束聚焦至目标段落；
  5. **完稿与复盘沉淀**：演说结束呈现「演说圆满完成」复盘卡，包含实际总用时、实测语速（WPM）、朗读段数及节奏评价，支持一键采纳实测语速沉淀为个人校准系数（`calibrationFactor`），闭环提升后续排稿与时长预估精准度；
- **盲操与舞台快捷微调**：

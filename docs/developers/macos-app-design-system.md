...
6. **命令与快捷键**：`SpeechRailCommands`（`App.swift`）提供 File/View/Help 菜单；14 个页面命令统一消费
   ...
   `⌘E` 通过 `FocusedValues.selectedWorkCommand` 绑定到当前场景选中的作品；`⌘R`（重新读取当前页）
    通过 `FocusedValues.reloadPageCommand` 绑定到**页面自己声明的**重读动作——所以头部不需要十四个
...
5. **模型档位 (Model Profiles)**：服务目录实际发布的档位构成选择集；轻快 / 品质 / 参考三档在宽度充足时一行展示，不足时按两列排成 2×2。
   ...
   两项不同即为「混合组合」：此时档位卡不单独高亮、事实行把总量标成「组合总大小」（按两档制品并集求和），「分别调整识别与配音」自动展开；点任一快捷卡会同时把两项写回该档并收起高级项，服务回填的当前/配置档位是混合组合时也走同一回填。下载与应用只提交这一对 `asr_spec`/`tts_spec`，不产生第三份「组合」真相。

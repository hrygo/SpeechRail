...
在“模型管理”中，可选择服务当前目录公布的 `extreme`、`quality`、`balanced` 或 `light` 档位后单独执行“下载并校验”。`extreme` 是候选档，未取得质量、资源和延迟证据前不应在正式运行时启用或宣称质量更高。下载页显示的模型总大小不是本次下载量；已校验的文件不会重下。下载操作只使用 SpeechRail 锁定的模型目录，逐文件校验大小和 SHA-256，并原子发布到本机；它不会自动应用 profile、启动/重启服务、删除已有模型，也不会上传音频或作品。确认“应用此档位”才会触发 profile 切换和现有服务验收。
...
客户端只调用公共 REST/WebSocket endpoint，不需要感知模型 worker 的加载、卸载或切换。选定的 profile 与请求 capability 会由服务端自动路由；`quality` 与候选 `extreme` 都可配置独立的 `voice_design` 与 `voice_clone` TTS worker。实际可用性以当前服务声明的能力为准。不同 lane 可以并发，同一 lane 的请求会按 worker lock 排队。空闲冷却导致 worker 回收时，服务会在下一次对应请求中惰性恢复，不改变客户端契约。

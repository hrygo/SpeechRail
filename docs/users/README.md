...
在“模型管理”中，可选择服务当前目录公布的 `fast`、`quality`、`reference` 三档规格，并分别指定识别（ASR）与配音（TTS）；两项目都选好后单独执行“下载并校验”。下载页显示的模型总大小不是本次下载量；已校验的文件不会重下。下载操作只使用 SpeechRail 锁定的模型目录，逐文件校验大小和 SHA-256，并原子发布到本机；它不会自动应用 profile、启动/重启服务、删除已有模型，也不会上传音频或作品。确认“应用”才会触发 profile 切换和现有服务验收。

...
> 上面第 3 段讲话人分离只有在当前实例显式供给分人资产并 ready 时才生效；未供给时调用返回
> `503 diarization_not_available`，`/v1/models` 也不列出 `gpt-4o-transcribe-diarize`。第 1 段的文件
> 转写与 `segment`/`word` 时间戳在任何档位都正常可用（ASR 原生提供）。

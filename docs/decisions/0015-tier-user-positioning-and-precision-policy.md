...
## 修订（2026-09-24，Q4 制品退役）

截至 2026-09-24，`asr-0.6b-q4` 与 `tts-0.6b-custom-q4` 已从当前模型 catalog 退役；现行 catalog 仅保留被档位使用的 Q8/BF16 制品。此前 2026-09-11 的“仍在 catalog、但不被档位引用”只描述当时状态，不再代表当前制品目录。此次本机清理将两个对应模型目录移入 macOS 废纸篓；不改动 Q8/BF16 模型、当前档位或历史测量记录。

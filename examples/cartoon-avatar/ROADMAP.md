...
当前状态：**R0 首版示例代码已实现；R1 功能实现已完成，并已通过自动化测试、本机浏览器 proxy smoke、真实 SpeechRail TTS smoke 和指定桌面/窄屏视口复测。** 两位未参与实现的独立试用者和系统减少动态效果现场复测仍未完成，不能据此宣称 R1 全部验收通过。本路线图不替代当前代码、README 和实际验收记录。R2–R5 的其余内容是后续候选迭代，不代表已经排期或授权实施。阶段编号是本示例的产品里程碑，与 SpeechRail 服务版本无关。
...
|---|---|---|---|
| **R0 可靠播报员** | 输入一段话，让卡通人物说出来 | 人物立绘、音色选择、完整 WAV 播放、停止和错误恢复 | 首版计划验收通过，建立真实体验基线 |
...
- 一个半身透明人物立绘，以及 `idle`、`thinking`、`speaking`、`emphasis`、`settle` 五种轻量动作；动作随播放状态和音量变化，但不声称提供精细唇形同步。
...
**验收门槛：** 以首版详细计划为准；特别检查迟到响应不得重新播放、停止和结束时人物回到稳定状态、密钥不进入浏览器、音频不落盘。
...
- 已验证：3 个原创内联 SVG 预设、profile 与期望 `voice ID` 联动；`serena` 不可用时不会静默替换，并要求手动选择替代音色。
- 已验证：本机 Codex In-app Browser `821×863` 中完成角色切换、键盘 `ArrowDown`/`Enter` 选角、替代音色播报、全文展示、音频时间进度和停止恢复；中等桌面布局无页面滚动溢出，浏览器 error/warn 日志为空。
- 已验证：自动化 R1 测试与布局断言通过；Chrome viewport `390×844`、`320×568` 和 `1440×900` 均无页面溢出，窄屏下角色舞台保持可见、控制面板可独立滚动；`prefers-reduced-motion` 已纳入 CSS/静态回归检查。
- 已验证：真实 `127.0.0.1:8201` 在本次复测中 `readyz=200`、`tts_ready=true`、9 个音色可用；完成 `midnight → uncle_fu` 的真实 TTS 播报，进度从生成/播放到 `00:03 / 00:03` 正常结束。
- 未验证：两位未参与实现的独立试用者尚未组织；系统级减少动态效果现场复测仍待补做。
...
当前待验证：R1 的独立试用者与系统减少动态效果现场复测，以及 R2 以后的流式收益、ASR/LLM/TTS 端到端延迟、扬声器回声场景和实际嵌入需求。其余阶段门槛仍为拟定条件，不能用 R1 的当前证据替代。

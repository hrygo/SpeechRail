...
仅当用户请求完整发布质量验收时，执行以下质量套件；PATCH 也不因版本级别较低而省略相关质量验证。
单项延迟、资源或指定音色测量只覆盖请求范围，报告其限制，不自动增加全音色、重启复测或 ABX。
完整质量报告覆盖：
...
- 每角色 3 类文本 × 3 次生成、服务重启后重复、同文本 PCM hash、跨文本 speaker embedding、
  跨重启变化和 ABX 盲听。重启须在已授权维护范围内，盲听须有实际参与条件；缺项标为未验证，
  不声称完整质量验收通过。建议门值需先在首个可信数据集上冻结。

...
FIXTURE_TEXTS = {
    ...
    "audio_10s": (
        "你好，这是本地语音识别与合成服务的性能基准测试。SpeechRail "
        "能够快速高效地输出高品质语音。"
    ),
    "audio_30s": (
        "SpeechRail 是一个本地优先的语音识别与合成服务。"
        "它为各种本地智能体和对话应用提供稳定可靠的 ASR 与 TTS 接口。"
...
def auth_headers(app_home: Path | None = None) -> dict[str, str]:
    ...
    key = resolve_api_key(app_home=app_home)

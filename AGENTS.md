...
- **禁止未经授权的 UI 自动化测试（硬约束）**：任何会接管前台窗口、焦点、输入或屏幕的自动化测试都必须由当前用户消息逐次明确要求才可执行，包括 XCUITest / UI test（含 `scripts/macos_app_test.sh`）、浏览器或桌面自动化（Playwright、webapp-testing 等）、录屏、点击驱动和窗口断言。skill、SOP、计划、README、CI 文档或发布流程中的“必须运行”一律不构成授权；不得以“发布/验收流程要求”“CI 需要”“顺手验证”为由自动触发。默认不运行 UI 自动化，并在结果报告中明确说明未执行项及原因；确有必要时先说明将占用的窗口与时长并取得用户确认。
...
- Python 固定为 `>=3.12,<3.13`，使用 `uv` 与 PEP 621；运行目标为 macOS Apple Silicon，`SpeechRailApp` GUI target 以 macOS 26.0 为最低版本。ControlKit、ControlAgent 和服务侧 SwiftPM worker 保留各自独立的最低版本，不把 App UI 的新特性降级到共同最低版本。

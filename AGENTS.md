...
- UI 界面文字按对应功能的目标受众组织：围绕用户要完成的任务、当前状态和下一步动作，优先使用普通用户能直接理解的用户语言；不得把内部实现、架构、模型或声学等专业术语直接作为主标题、按钮和状态文案，技术术语仅在确有必要且有上下文的技术层或开发者详情中使用。
- 所有面向用户的流程默认按“傻瓜化”设计：首屏和默认路径只呈现完成当前任务所需的少量明确动作，先告诉用户发生了什么、是否需要确认以及下一步做什么；拆分、合并、来源区间、模型/协议、诊断和其他高级操作采用渐进式披露，放入命名明确的菜单、折叠区或开发者详情。隐藏高级操作不得让普通路径失去可见、可键盘访问或可无障碍访问的常用能力。
...
- 根 `README.md` 只保留价值、当前公共能力、快速开始和文档入口；实现细节、基准与操作步骤进入专业文档。修改任一语言版本的根 README 时，同步调整另一语言版本的对应内容，保持双语一致；除非用户明确要求，不因发布、接口或普通实现变化自动触发 README 更新或双语同步。

"""Small durable idempotency journal for local destructive/create operations."""
...
class DurableIdempotencyJournal:
    ...
    def _load_locked(self) -> list[dict[str, object]]:
        ...
        try:
            ...
            for item in raw:
                ...
                required = {"owner", "operation", "key_hash", "fingerprint", "state", "created_at"}
            ...
            raise IdempotencyStoreUnavailableError("idempotency journal is unavailable") from exc
                ...
                if (
                    ...
                    if state == "completed":
                        ...
                        return IdempotencyDecision(
                            "completed", str(result) if isinstance(result, str) else None
                        )
                    return IdempotencyDecision("pending")
            records.append(
                {
                    "owner": owner,
                    "operation": operation,
        ...
        with self._lock:
            ...
            for record in reversed(records):
                if (
                    ...
                    self._save_locked(records)
                    return result_id
            raise IdempotencyStoreUnavailableError("pending idempotency record is missing")
    ...
    def abort(
        ...
        *,
        ...
        """Remove only a matching pending record when no external side effect occurred."""

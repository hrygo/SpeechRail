...
手动 PCM 提交、可选原生 Qwen3 连续 ASR、ordered audio delta、TTS response 取消和基础容量
隔离已有 deterministic fake-backend 测试。没有已授权且通过 smoke 的本地流式 ASR/TTS worker
时，服务仍会返回 `backend_not_ready`；客户端正式切换和端口退役仍须另行授权。
